...
from G_MLZSL.util import weights_init
...
class CLF(nn.Module):
    def __init__(self, args):
        ...
        self.res_size, self.N, self.hidden_size, late_heads = args.res_size, args.N, args.hidden_size, 8
        self.early_fusion = generator(args, args.att_size)
        self.late_fusion = generator(args, args.att_size)
        self.attn = Fusion_Attention(late_heads, self.res_size, self.hidden_size)

    def forward(self, noise, att, avg_att):
        late_out = torch.zeros(len(att), self.res_size).cuda()
        idx_count = torch.zeros(len(att)).cuda()
        for j in range(att.size(1)):
            idx = [i for i in range(len(att)) if att[i, j].abs().sum() > 0]
            idx_count[idx] += 1
            late_out[idx] += torch.sigmoid(self.late_fusion(noise[idx], att[idx, j]))
...
class Discriminator(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = nn.Linear(args.res_size + args.att_size, args.ndh)
